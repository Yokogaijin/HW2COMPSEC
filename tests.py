...
def permuted_choice_1(key_bin):
    return permute(key_bin, PC1)
...
# DES S-boxes (S1 - S8) and Expansion (E)
def f_function(right, round_key):
    ...
    for i in range(8):
        ...
        col = int(block[1:5], 2)
        sbox_output += f"{S_BOX[i][row][col]:04b}"
...
def generate_round_keys(key_bin):
    ...
    key_plus = permuted_choice_1(key_bin)
    round_keys = []
...
def des_decrypt(ciphertext_bin, key):
    ...
    # Initial permutation
    permuted_text = initial_permutation(ciphertext_bin)
    print(f"Initial Permutation: {permuted_text}")
    ...
    for i, round_key in enumerate(round_keys):
        new_right = xor(left, f_function(right, round_key))
        left = right
        right = new_right
        print(f"Round {i+1}:")
    ...
    decrypted_text = final_permutation(combined_text)
    print(f"Final Permutation: {decrypted_text}")
    
    plaintext = binary_to_text(decrypted_text)
